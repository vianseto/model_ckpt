...
avatar_dir = "/data/botika/eef58538-68b7-42bd-a60c-73abbba6834a/home/botika/vian/sd/app/avatar"

def wav2lip_gan(face_path, audio_path, output_path):
    torch.cuda.empty_cache()
    !python /media/botika/eef58538-68b7-42bd-a60c-73abbba6834a/home/botika/vian/Wav2Lip/inference.py \
        --checkpoint_path="/media/botika/eef58538-68b7-42bd-a60c-73abbba6834a/home/botika/vian/Wav2Lip/checkpoints/wav2lip_gan.pth" \
        --face=$face_path \
        --audio=$audio_path \
        --outfile=$output_path \
        --nosmooth --fps=30
...
def real_esrgan_video(video_path, output_path, scale):
    torch.cuda.empty_cache()
    !python /media/botika/eef58538-68b7-42bd-a60c-73abbba6834a/home/botika/vian/Real-ESRGANinference_realesrgan_video.py \
        -n RealESRGAN_x4plus \
        -i=$video_path \
        -o=$output_path \
        --outscale=$scale \
    ...
    if request.method == 'POST':
        ...
        if os.path.isfile(template_path)!=True:
            ...
            try:
                ...
                if enhance:
                    cap = cv2.VideoCapture(output_dir)
                    ...
                    try:
                        real_esrgan_video(output_dir, id_dir, upscale_value)

                        old_file_name = os.path.join(id_dir,video+"_out.mp4")
                        new_file_name = output_dir

                        os.rename(old_file_name, new_file_name)
                    except:
                        espond = {'msg': "out of memory"}
                        return json.dumps(respond, indent=2)
...
@app.route('/avatar/video/enhance', methods=['POST'])
def avatar_video_enhance():
    #log_input_request(request.get_json(), "avatar_video_enhance.txt", request.url, request.method)
    if request.method == 'POST':
        # Sanitasi ID
        if not isinstance(request.json['id'],str):
            respond = {'msg': 'id value must be string'}
        ...
        if os.path.exists(id_dir) != True:
            ...
            # Output path
            output_path = os.path.join(id_dir,"upscale")
            ...
            try:
                real_esrgan_video(video_path, output_path, upscale_value)

                old_file_name = os.path.join(output_path,video+"_out.mp4")
                new_file_name = os.path.join(output_path,video+"_enhance.mp4")

                os.rename(old_file_name, new_file_name)

            except:
                respond = respond = {'msg': "out of memory"}
                return json.dumps(respond, indent=2)
            else:
                respond = {'result': "success", 'id': id, 'video':video+"_enhance"}
                return json.dumps(respond, indent=2)              
    else:
        respond = {'msg': 'please use POST method'}
        return json.dumps(respond, indent=2)
...
if __name__ == '__main__':
    app.run(threaded=True, processes=2, host='0.0.0.0', port='8000')