...
def wav2lip_gan(face_path, audio_path, output_path):
    ...
    os.system(f"python /data/server/Wav2Lip/inference.py --checkpoint_path /media/botika/eef58538-68b7-42bd-a60c-73abbba6834a/home/botika/vian/Wav2Lip/checkpoints/wav2lip_gan.pth --face={face_path} --audio={audio_path} --outfile={output_path} --nosmooth --fps=30")

def real_esrgan_video(video_path, output_path, scale):
    torch.cuda.empty_cache()
    os.system(f"python /data/server/Real-ESRGAN/inference_realesrgan_video.py -n RealESRGAN_x4plus -i={video_path} -o={output_path} --outscale={scale} --face_enhance --fps 30")
